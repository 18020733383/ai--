...
const shaperSchema: Schema = {
  ...
  properties: {
    ...
    troop: {
      ...
      required: ["name", "race", "tier", "basePower", "maxXp", "upgradeCost", "description", "equipment", "attributeRatios"],
    }
  },
  ...
  properties: {
    ...
    troops: {
      ...
      items: {
        ...
        required: ["tier", "name", "basePower", "maxXp", "upgradeCost", "description", "equipment", "attributeRatios"]
      }
    }
  },
...
4) attributeRatios 六项为 0~1 的小数（attack/defense/agility/hp/range/morale），六项之和必须为 1，表示各属性占比；系统会根据 tier 基线自动计算实际数值，禁止直接写整数属性。
...
const normalizeAltarTroops = (troops: AltarTroopDraft[], fallback?: AltarTroopDraft[]) => {
  ...
  /** 仅使用 attributeRatios 按 tier 基线分配属性，禁止 AI 直接返回整数属性，防止 0 或极端值 */
  const resolveAttributes = (troop: AltarTroopDraft, tier: number, fallbackValue: AltarTroopDraft['attributes']) => {
    const ratios = (troop as any).attributeRatios;
    if (ratios && typeof ratios === 'object') {
      const hasAny = ['attack', 'defense', 'agility', 'hp', 'range', 'morale'].some(k => ratios[k] !== undefined);
      if (hasAny) return attributesFromRatios(tier, ratios);
    }
  ...
  const normalizeOne = (troop: AltarTroopDraft, fallbackTroop: AltarTroopDraft | undefined, fallbackTier: number) => {
    ...
    const attributes = resolveAttributes(troop as any, tier, fallbackTroop?.attributes ?? fallbackAttributes(tier));
...
troops 的每个元素字段必须为 tier, name, basePower, maxXp, upgradeCost, description, equipment, attributeRatios；slot 可选。
equipment 必须是字符串数组。
attributeRatios 必须包含 attack, defense, agility, hp, range, morale，全部为 0~1 的小数且六项之和为 1，表示各属性占比；系统会按 tier 基线分配实际数值。
  ...
  "troops": [
    {
      ...
      "description": "基础教徒。",
      "equipment": ["潮汐草叉","亚麻布衣"],
      "attributeRatios": { "attack": 0.22, "defense": 0.15, "agility": 0.19, "hp": 0.22, "range": 0.04, "morale": 0.18 }
    }
  ]
}
...
export const proposeShapedTroop = async (
  ...
  const lastSummary = last ? (() => {
    ...
    return [
      ...
      `- 上次属性: ${(() => { const a = (troop as any).attributes; const r = (troop as any).attributeRatios; const attrs = a ?? (r && typeof r === 'object' ? attributesFromRatios((troop as any).tier ?? 2, r) : null); return attrs ? `攻${attrs.attack} 防${attrs.defense} 敏${attrs.agility} 体${attrs.hp} 远${attrs.range} 士${attrs.morale}` : '（无）'; })()}`,
      `- 上次装备: ${equip}`,
...
4) 如果可以制作，返回 decision="OK" 且提供 troop 模板：
   ...
   - equipment: 1-5 个装备名
   - attributeRatios: 六维占比（attack/defense/agility/hp/range/morale），全部为 0~1 小数且六项之和为 1，必须随 tier 提升而提高，且不应低于该 tier 的“基准属性”太多
   - upgradeTargetId 可为空字符串或省略
4.1) 示例比例：偏攻击型 attack:0.25 defense:0.15 agility:0.2 hp:0.2 range:0.05 morale:0.15；偏坦克型 attack:0.15 defense:0.3 agility:0.1 hp:0.25 range:0.05 morale:0.15
5) 价格参考（地图招募基础价，作为参照）：
   - Tier 1: 10-30
   - Tier 2: 40-80
   - Tier 3: 120-250
